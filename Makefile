...
# operating system:  Linux 4.19.0-6-amd64 x86_64 [ELF] (4.19.0)
# libc:	(6.28.0)
# binutils:	(231)
...
DEPLIBS =
LOCAL_LIBRARIES = $(XAWLIB) $(XMULIBONLY) $(XTOOLLIB) $(XPMLIB) $(EXTRAXAWCLIENTLIBS) $(XLIB)
...
install:: seyon
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  seyon $(DESTDIR)$(BINDIR)/seyon

all:: seyon.$(MANNEWSUFFIX)

seyon.$(MANNEWSUFFIX): seyon.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname seyon` && \
	   $(LN) `basename seyon.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < seyon.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) seyon.$(MANNEWSUFFIX)

install.man:: seyon.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) seyon.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/seyon.$(MANSUFFIX)
...
install:: Seyon.ad
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) Seyon.ad $(DESTDIR)$(XAPPLOADDIR)/Seyon
...
install:: Seyon-co.ad
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) Seyon-co.ad $(DESTDIR)$(XAPPLOADDIR)/Seyon-color
...
install:: seyon.help
	@if [ -d $(DESTDIR)/usr/share/seyon ]; then \
		set +x; \
	else \
		if [ -h $(DESTDIR)/usr/share/seyon ]; then \
			(set -x; rm -f $(DESTDIR)/usr/share/seyon); \
		fi; \
		(set -x; $(MKDIRHIER) $(DESTDIR)/usr/share/seyon); \
	fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTDATFLAGS) seyon.help $(DESTDIR)/usr/share/seyon/seyon.help
